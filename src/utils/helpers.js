...
function renderStreakProgress(containerId) {
    ...
    if (p.nextMilestoneDay != null && p.percentProgress != null) {
        el.innerHTML = [
            ...
            '<div class="next-milestone-label">Next: Day ' + p.nextMilestoneDay + ' — ' + nextLabel + '</div>',
            '<div class="next-milestone-bar"><div class="next-milestone-fill" style="width:' + p.percentProgress + '%"></div></div>',
            '<div class="next-milestone-meta">' + p.daysRemaining + ' day' + (p.daysRemaining === 1 ? '' : 's') + ' left · ' + p.percentProgress + '%</div>',
            '</div>'
        ].join('');
        el.style.display = '';
